...
// Arreglos para calses reutilizables
...
void setup() {
  ...
  // Inicializar objetos
  // Inicializar objetos de ElementoFlotante con distintas imágenes
...
void draw() {
  ...
  if (estado.equals("PORTADA")) {
    ...
    if (mouseX > cartaX && mouseX < cartaX + cartaW &&
      ...
      // Mostrar ilustración de corazones (abajo del poema)
...
void mouseMoved() {
  if (estado.equals("AMOR")) {
    if (mouseX > cartaX && mouseX < cartaX + cartaW &&
      ...
      mostrarTexto = false;
      for (int i = 0; i < numCorazones; i++) {
        corazones[i] = new Corazon(-100, -100);
      }
      corazonIndex = 0;
    }
  }
...
class FiguraAleatoria {
  ...
  void actualizar(float[] spectrum) {
    int i = int(random(bands));
    float val = spectrum[i] * 1000;
